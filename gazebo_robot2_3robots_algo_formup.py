...
def check_robot3(msg):
	...
	global R3X
	global R3Y
	R3X = float(msg.pose.pose.position.x)
	R3Y = float(msg.pose.pose.position.y)
	distance = math.sqrt((R3X-MyX)**2 + (R3Y-MyY)**2)
...
def listener():
	...
	rospy.Subscriber("/robot2/scan", LaserScan, check_laser)
	rospy.Subscriber("/robot2/odom", Odometry, get_rotation_pos)
	rospy.Subscriber("/robot1/odom", Odometry, check_robot1)
	rospy.Subscriber("/robot3/odom", Odometry, check_robot3)
	rospy.Subscriber("/robot2/odom", Odometry, get_rotation_pos)
	rospy.Subscriber("/robot2/odom", Odometry, go)
	rospy.spin()
